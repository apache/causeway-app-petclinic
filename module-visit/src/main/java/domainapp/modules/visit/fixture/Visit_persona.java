        ...
        protected Visit buildResult(final ExecutionContext ec) {
            ...
            petOwner.getPets().forEach(pet -> {
                ...
                for (var i = 0; i < numVisits; i++) {
                    ...
                    wrapperFactory.wrapMixin(PetOwner_bookVisit.class, petOwner, SyncControl.defaults().withSkipRules()).act(pet, appointmentTime);
                }
            ...
            final var visits = wrapperFactory.wrapMixin(PetOwner_visits.class, petOwner, SyncControl.defaults().withSkipRules()).coll();
            return lastOf(visits);
        }
